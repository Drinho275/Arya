...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Drinho gostoso\n' // Nama kamu
            + 'ORG:Arya;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=554999156302:+55 49 9915-6302\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '/','*','.'
blocked = []
limitawal = '999999999'
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Flw @${num.split('@')[0]} Espero que possa voltat `
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD Arya BOT*',
					...
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de drinho. Registre-se para fazer amizade com arya por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar drinho|14\n\n──「 ৡৢ͜͡𝙰𝚛𝚢𝚊 𝚋𝚘𝚝 」──`,
			...
			const ownerNumber = ["554999156302@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"554999156302@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["554999156302@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["554999156302@s.whatsapp.net"]//ubah nomor lo
			const premium = ["554999156302@s.whatsapp.net","5583998414396@s.whatsapp.net","556796558564@s.whatsapp.net","559295205191@s.whatsapp.net","557488746593@s.whatsapp.net","554792091566@s.whatsapp.net","558699541889@s.whatsapp.net","559294313229@s.whatsapp.net","554298653614@s.whatsapp.net"]
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("Arya não gosta de racismo, que isso sirva de exemplo 🚶")
		}, 4000)
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("2 segundos")
  	...
  	if (messagesC.includes("Bot")){
			client.updatePresence(from, Presence.composing)
			reply("Não me chame de bot, me chame de arya, por favor💖")
	}		
	  ...
	  if (messagesC.includes("zuckerbeg")){
			client.updatePresence(from, Presence.composing)
			reply("Um viado que só me da ban 🤬")
	}
		...
		if (messagesC.includes("sticker")){
			client.updatePresence(from, Presence.composing)
			reply("já vou fazer sua Figurinha ❤️")
	}	
		...
		if (messagesC.includes("cadearya")){
			client.updatePresence(from, Presence.composing)
			reply("olha eu aqui carai")
	}
	
		if (messagesC.includes("arya")){
			client.updatePresence(from, Presence.composing)
			reply("roi, falando de mim?🥺❤️")
	}
	  if (messagesC.includes("drinho")){
			client.updatePresence(from, Presence.composing)
			reply("meu criador ai 💖")
				}
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIO Arya BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @nabuto domina vida
          ...
          reply('Espere..')
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT DRINHO* ✨* 」\n*│+ wa.me/554999156302*╰──────*「 *NABUTO* 」*────*\n\n*_SE QUER SER ADMIN DA ARYA BOT_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
                ...
                break
                 ...
                 case 'calculadora':
					...
					buffer = await getBuffer(`https://www.bing.com/images/search?view=detailV2&ccid=fvfBMOyR&id=D848DFA2D81ED74B902F7B36507B6420FD0987EC&thid=OIP.fvfBMOyRry8VkdUmzZ6GkgHaF1&mediaurl=https%3a%2f%2fth.bing.com%2fth%2fid%2fR7ef7c130ec91af2f1591d526cd9e8692%3frik%3d7IcJ%252fSBke1A2ew%26riu%3dhttp%253a%252f%252fwww.ovale.com.br%252f_midias%252fjpg%252f2018%252f03%252f08%252fboi1-173065.jpg%26ehk%3doyUuQwyaIKFzso1Fh4AeDQPayvwzTCCiyuGV%252f3XBxLE%253d%26risl%3d%26pid%3dImgRaw&exph=680&expw=864&q=boi&simid=607993453744292908&ck=316D4C555DCB6B0F5269A742C47F0DE5&selectedIndex=3&FORM=IRPRST&PC=IFJ4`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Drinho ai affs🙄'})
					break
				...
				case 'porno':
					...
					buffer = await getBuffer(`https://www.bing.com/images/search?view=detailV2&ccid=gP9Ql4Xn&id=5D28A29A6DE55066BFDAD244DD8E7E3B07500680&thid=OIP.gP9Ql4XnBXioOAbUzTSkmQHaLH&mediaurl=https%3a%2f%2fstatic-ca-cdn.eporner.com%2fphotos%2f616307.jpg&exph=1920&expw=1280&q=%0d%0aLana+rodes%0d%0anua%0d%0a&simid=608035694767704943&ck=E0D3966A258AD8B51153658DA3FBB5F2&selectedIndex=1&FORM=IRPRST`)
        ...
        case 'drinho':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://www.bing.com/images/search?view=detailV2&ccid=Q5%2fACtkA&id=8AE22898F7C44BD8D382A656FE7B3F21DF4009ED&thid=OIP.Q5_ACtkATTlGFj72WnvoSgHaFj&mediaurl=https%3a%2f%2fi0.wp.com%2fdavidarioch.com%2fwp-content%2fuploads%2f2018%2f02%2fO-boi-fuj%c3%a3o.jpg&exph=768&expw=1024&q=boi%0d%0a&simid=608038770030215234&ck=A200CBAA8AC58D999531D8915621D7DA&selectedIndex=4&FORM=IRPRST`)
