...
const { limit } = require('.//menu/limit*/
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `@${num.split('@')[0]} 𝙎𝙖𝙞𝙪, 𝙨𝙚𝙧𝙖 𝙦𝙪𝙚 𝙚𝙡𝙚 𝙣𝙖𝙤 𝙜𝙤𝙨𝙩𝙤𝙪 𝙙𝙚 𝙢𝙞𝙢? 🥺`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
	...
	if (messagesC.includes("boot")){
			client.updatePresence(from, Presence.composing)
			reply("Não é boot, é bot 🙄 e meu nome é Arya")
	}
	  ...
	  if (messagesC.includes("drinho")){
			...
			client.updatePresence(from, Presence.composing)
			data = fs.readFileSync('./src/drinhojokes.js');
...
//@nabutoYT					
    ...
    break
				...
				case 'nulis'
				case 'tulis':
					if (args.length < 1) return reply('Quer anotar qual texto?')
...
// recursos anime, random @drinho domina vida
          ...
          reply('Espere..')
			    ...
			    case 'dono':
                    ...
                    client.sendMessage(from, 'Meu dono é esse gostoso aí tal de Drinho 🙄, chama ele aí ele tem a voz gostosa',MessageType.text, { quoted: mek} )
                    break
                case 'fitnah':	
				case 'fake':          
                    if (!isGroup) return reply(mess.only.group)
			  ...
			  case 'fotodrinho':
					...
					buffer = await getBuffer(`https://i.imgur.com/1MzjNud.jpg`)
        ...
        case 'fotodrinho2':
				...
				case 'kinechan':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/xLNzRXZ.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Delicia 😍'})
					break
				case 'porno':
				    if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/po5qwMv.jpg`)
					buffer = await getBuffer(`https://i.imgur.com/5j4pABK.jpg`)
					buffer = await getBuffer(`https://i.imgur.com/rKGssOA.jpg`)
					buffer = await getBuffer(`https://i.imgur.com/3Id7QRa.jpg`)
					buffer = await getBuffer(`https://i.imgur.com/s9zJej9.jpg`)
					buffer = await getBuffer(`https://i.imgur.com/GfI8p9M.jpg`)
					buffer = await getBuffer(`https://i.imgur.com/SdpIXqP.jpg`)
					break
				case 'belle2':
					memein = await kagApi.memeindo()
